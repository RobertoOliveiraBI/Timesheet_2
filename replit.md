...
- **Detailed Weekly Approval View with Batch Actions (2025-10-23)**: Implemented comprehensive weekly approval interface (AprovacaoSemanalDetalhada.tsx) for managers with granular entry-level control and batch approval capabilities. Features include: automatic selection of first team member on load; collaborator filter dropdown; week navigation with date range display; detailed table showing individual entries by day with status indicators (R=Rascunho/gray, E=Enviado/amber, A=Aprovado/green); individual action buttons per entry (comment icon to view/add comments with visual indicator - filled blue icon when comments exist, approve for VALIDACAO status, return to draft for APROVADO status, delete for non-approved entries); batch approval for entire row (all entries in a specific client/campaign/task combination); batch approval for entire week (all pending entries for selected collaborator); confirmation dialogs showing entry counts and total hours; integrated CommentModal for manager feedback and collaborator responses directly from the approval table with automatic comment detection and icon update; 100-line display limit with warning message; totals per day and grand total row. Backend enhanced with date range (fromDate/toDate) and user ID filtering on /api/approvals/pending endpoint. This view replaced the previous ApprovalManagement component as the main approval interface at /approvals route.
- **Reports Statistics Pagination Fix (2025-10-23)**: Fixed critical bug in ReportsSection where hour statistics (Total Horas, Horas Faturáveis, Horas Não Faturáveis) were calculated only from paginated entries (100 per page) instead of all filtered entries. Modified query to calculate statistics on complete filtered dataset before pagination, ensuring accurate totals regardless of pagination state.
- **Dashboard Hours by Task Type Enhancement (2025-10-23)**: Converted "Quantidade de Lançamentos por Tipo" chart to "Soma de Horas por Tipo" in MetricsDashboard. Backend now sums hours instead of counting entries for each task type. Added detailed table below the pie chart showing task type, hours (formatted with 2 decimals), and percentage, sorted by hours descending. Table includes color indicators matching the pie chart segments.
